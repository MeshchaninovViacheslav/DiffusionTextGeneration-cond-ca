...
class DiffusionRunner:
    def __init__(
        ...
        # Decoder
        self.decoder = BertDecoder(model_name=config.model.encoder_name, mode=config.model.decoder_mode, is_cond=config.model.decoder_is_cond)
    ...
    def generate_text_batch(self, batch_size, cond=None, attention_mask=None):
        ...
        if self.config.dynamic.solver == "euler":
            solver = self.pred_embeddings
        elif self.config.dynamic.solver == "ddpm":
            solver = self.pred_embeddings_ddpm
        elif self.config.dynamic.solver == "ddim":
            solver = self.pred_embeddings_ddim

        pred_embeddings = solver(
            batch_size,
            attention_mask=attention_mask,
            cond_x=cond_x,
    ...
    def estimate(self):
        ...
        if dist.get_rank() == 0:
            ...
            max_order = 4
            bleu = compute_bleu(predictions=predictions, references=references, max_order=max_order)
            ...
            self.log_metric(metric_name=f"BLEU-{max_order}", loader_name="", value=bleu)

            for rouge_type in ['1', '2', 'L']:
                print(f"Rouge-{rouge_type}: {metrics_rouge[f'rouge{rouge_type}']:0.5f}")
            print(f"BertScore: {bertscore:0.5f}")
            print(f"BLEU-{max_order}: {bleu:0.5f}")
    ...
    def pred_embeddings_ddpm(
        ...
        def q_x_t_rev(x_t, x_0, t, next_t):
            alpha_t = self.dynamic.marginal_params(t)["mu"] ** 2
            alpha_t_1 = self.dynamic.marginal_params(next_t)["mu"] ** 2
            beta_t = 1 - alpha_t / alpha_t_1
        ...
        with torch.no_grad():
            ...
            for idx in tqdm(range(self.dynamic.N)):
                ...
                mu, std = q_x_t_rev(x_t, x_0, input_t, next_input_t)
                x_t = mu + std * torch.randn_like(x_t)
                x_0_self_cond = x_0
