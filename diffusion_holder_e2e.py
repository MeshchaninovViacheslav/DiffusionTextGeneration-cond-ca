...
class DiffusionRunner:
    ...
    def calc_loss(
        ...
        loss = self.config.loss.ce_enc_coef * ce_loss + \
               self.config.loss.l0_coef * loss_x_0 + \
               self.config.loss.ce_dif_coef * ce_loss_pred
    ...
    def train_epoch(self):
        for _, X in enumerate(self.train_loader):
            ...
            self.train_range.set_description(
                ...
                f"clean_x_mean: {stat_dict['clean_x_mean'].item():0.4f}, "
                f"clean_x_var: {stat_dict['clean_x_var'].item():0.4f}, "
            )
